...
class Optimisation(object):
    ...
    def __init__(self):
		...
		self.connection3 = pika.BlockingConnection(
            pika.ConnectionParameters(host='localhost'))	
        ...
        result1 = self.channel1.queue_declare(queue = 'frontend_optimisation')
        result2 = self.channel2.queue_declare(queue = 'optimisation_onemap')
		result3 = self.channel3.queue_declare(queue = 'recommendation_optimisation')

        self.callback_queue1 = result1.method.queue
        self.callback_queue2 = result2.method.queue
		self.callback_queue3 = result3.method.queue

        self.channel1.basic_qos(prefetch_count=1)

        self.channel1.basic_consume(queue='frontend_optimisation', on_message_callback=self.on_request)
		self.channel3.basic_consume(queue='recommendation_optimisation', on_message_callback=self.on_request)
    ...
    def call(self, n):
        ...
        self.channel2.basic_publish(
            exchange='',
            routing_key='optimisation_onemap',
...
optimisation.channel1.start_consuming()